export const projects = [
  {
    ...
    description: "Eyeball is a React Native mobile app (live on the App Store) built on the Expo framework that transforms how you save and interact with bookmarks. The app features automatic metadata scraping to display rich bookmark cards, custom organization via channels, and an innovative chatbot that enables AI conversations about your saved links. \n\nBuilt with React Native frontend and Express.js backend hosted on Railway. Uses Supabase for PostgreSQL database and authentication. Integrates with OpenRouter for AI functionality and the iOS share sheet for seamless link saving across iOS.",
